...
public class TaskGLoop {
    ...
    private static int calculateQty(int weight, int pieceWeight, int partWeight) {
        ...
        while (weight >= pieceWeight) {
            int pieceQty = weight / pieceWeight;
            weight %= pieceWeight;
            partQty += pieceQty * (pieceWeight / partWeight);
            weight += pieceQty * (pieceWeight % partWeight);
        }
